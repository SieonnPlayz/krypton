...
local KryptonRFolder = "KryptonData"
local ConfigurationFolder = KryptonRFolder.."/Configs"
local ConfigurationExtension = ".kr"



local KryptonRLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across KryptonR
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across KryptonR
...
local KryptonR = game:GetObjects("rbxassetid://10804731440")[1]

KryptonR.Enabled = false


if gethui then
	KryptonR.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(KryptonR)
	KryptonR.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	KryptonR.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	KryptonR.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == KryptonR.Name and Interface ~= KryptonR then
			Interface.Enabled = false
			Interface.Name = "KryptonR"
		end
	end
else
...
local Main = KryptonR.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

KryptonR.DisplayOrder = 100
...
local Notifications = KryptonR.Notifications

local SelectedTheme = KryptonRLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = KryptonRLibrary.Theme[ThemeName]
	for _, obj in ipairs(KryptonR:GetDescendants()) do
	...
	KryptonR.Main.BackgroundColor3 = SelectedTheme.Background
	KryptonR.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	KryptonR.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	KryptonR.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	KryptonR.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	KryptonR.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	KryptonR.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if KryptonRLibrary.Flags[FlagName] then
			spawn(function() 
				if KryptonRLibrary.Flags[FlagName].Type == "ColorPicker" then
					KryptonRLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if KryptonRLibrary.Flags[FlagName].CurrentValue or KryptonRLibrary.Flags[FlagName].CurrentKeybind or KryptonRLibrary.Flags[FlagName].CurrentOption or KryptonRLibrary.Flags[FlagName].Color ~= FlagValue then KryptonRLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			KryptonRLibrary:Notify({Title = "Flag Error", Content = "KryptonR was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(KryptonRLibrary.Flags) do
...
function KryptonRLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= KryptonRLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("KryptonR | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if KryptonR.Name == "KryptonR" then
...
function Hide()
	Debounce = true
	KryptonRLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function KryptonRLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "KryptonR Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Krypton"
	if Settings.LoadingTitle ~= "Krypton Renderer" then
		LoadingFrame.Version.Text = "KryptonR UI"
	end
	...
	pcall(function()
		...
		if not isfolder(KryptonRFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(KryptonRFolder.."/Discord Invites") then
			makefolder(KryptonRFolder.."/Discord Invites")
		end
		if not isfile(KryptonRFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(KryptonRFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"KryptonR RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(KryptonRFolder.."/Key System") then
			makefolder(KryptonRFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("KryptonR | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(KryptonRFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(KryptonRFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(KryptonRFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						KryptonRLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("KryptonR | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					KryptonRLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("KryptonR | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("KryptonR | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					KryptonRLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("KryptonR | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					KryptonRLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("KryptonR | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					KryptonRLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("KryptonR | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					KryptonRLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function KryptonRLibrary:Destroy()
	KryptonR:Destroy()
end
...
function KryptonRLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				KryptonRLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, KryptonRLibrary.LoadConfiguration, KryptonRLibrary)
if KryptonR:FindFirstChild("Notice") then
	KryptonR.Notice.Visible = true
	KryptonR.Notice.Interact.MouseButton1Click:Connect(function()
		KryptonR.Notice.Visible = false
	end)
end

	return KryptonRLibrary
